        ...
        <div className="detail-content">
          <section className={`action-panel ${project.stepState === "FAILED" ? "action-failed" : project.canRecover ? "action-stale" : isComplete ? "action-complete" : ""}`} aria-labelledby="action-title" aria-live="polite">
            {isComplete ? (
              ...
              <><div className="action-message"><span className="action-icon danger"><AlertIcon /></span><div><p className="eyebrow">Generation paused</p><h2 id="action-title">{runningConfig?.label ?? "This step"} needs another try.</h2><p>{project.lastError ?? "The generation request did not finish. Completed work has not been changed."}</p></div></div><button className="button button-primary" type="button" onClick={() => activeStep && onRunStep(activeStep)} disabled={actionPending}>{actionPending ? <><Spinner /> Retrying…</> : <>Retry {runningConfig?.label ?? "step"} <ArrowRightIcon /></>}</button></>
            ) : project.stepState === "RUNNING" || actionPending ? (
              <div className="running-panel"><span className="running-orbit"><SparkleIcon /></span><div><p className="eyebrow">Step {project.completedStep + 1} of 5 is running</p><h2 id="action-title">{runningConfig?.runningLabel ?? "Working on your story"}…</h2><p>Long Gemini calls can take a minute. You can leave safely—this work continues and the page will resume here.</p></div><span className="running-badge"><span /> Live</span></div>
            ) : currentStep ? (
            ...
            {actionError && <div className="inline-alert" role="alert"><AlertIcon /> {actionError}</div>}
          </section>
